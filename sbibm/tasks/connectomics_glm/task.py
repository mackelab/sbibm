...
class ConnectomicsGLM(Task):
    def __init__(self, upper_rate_bound=100000):
        """Bernoulli GLM

        A Bernoulli GLM model for generation of synapse counts given structural features
        ...
        super().__init__(
            ...
            name_display="Bernoulli Connectomics GLM",
    ...
    def get_simulator(self, max_calls: Optional[int] = None) -> Simulator:
        ...
        def simulator(parameters):
            log_dso = self.design_matrix.mm(parameters.T).T
            probs = 1 / (1 + torch.exp(-log_dso))
            data = pyro.sample("data", pdist.Bernoulli(probs=probs))
            return data
...
if __name__ == "__main__":
    task = ConnectomicsGLM()
    task._setup(create_reference=True, n_jobs=-1)

    # xs = []
    # seeds = []
    # for i in range(70):
    #     torch.manual_seed(1000000 + i)
    #     np.random.seed(1000000 + i)
    #     x = task.get_simulator()(task.get_prior()())

    #     if x.mean() > 0 and x.mean() < 0.8:
    #         xs.append(x)
    #         seeds.append(1000000 + i)
    # xs = torch.stack(xs)
    # xs, inverse = torch.unique(xs, dim=0, sorted=False, return_inverse=True)
    # seeds = torch.tensor(seeds)[torch.unique(inverse)]
    # print((seeds))
