...
class DDM(Task):
    def __init__(
        ...
        Args:
            ...
            dim_parameters: if 4 the simple DDM with non-decision time parameter is
                used, as in Fengler et al. 2021. If 3, ndt is ignored. If 2, the DDM
                with only drift and boundary separation as parameters is used.
        """
        self.dt = dt
        self.num_trials = num_trials
        assert dim_parameters in [2, 3, 4], "dim_parameters must be 2, 3, or 4."

        observation_seeds = torch.arange(0, 10, 1).tolist()
        ...
        super().__init__(
            ...
            num_simulations=[100, 10000, 100000],
    ...
    def get_simulator(
        ...
        if self.dim_parameters == 2:
            ...
            def simulator(parameters):
                ...
                # Add ndt to simulated rts as offset.
                rts += ndt.reshape(-1, 1)
                # encode zero choices as negative RTs.
                rts[choices == 0] *= -1
                return torch.tensor(rts, dtype=torch.float32)
    ...
    def _setup(self, n_jobs: int = -1, create_reference: bool = True, **kwargs: Any):
        ...
        def run(num_observation, observation_seed, **kwargs):
            ...
            simulator = self.get_simulator(seed=int(observation_seed))
